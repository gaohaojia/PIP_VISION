...
class YoLov5TRT(object):
    ...
    def infer(self, input_image_path):
        ...
        tag_size = 0.05
        tag_size_half = 0.02
        half_Weight=[229/4,152/4,40/4]
        half_Height=[126/4,142/4,40/4]
        fx = 1056.4967111
        fy = 1056.6221413136
        cx = 657.4915775667
        cy = 508.2778608
...
# position = -np.matrix(rotM).T * np.matrix(tvec)
            ...
            print(result_scores,mindex)
            label = str(categories[int(result_classid[mindex])] +" distance:" + str(distance) + 'cm conf:' + str(int(result_scores[mindex]*100)) + '%')
            print(f"label:{label}")
            try:
                plot_one_box(box, image_raw, label="{}".format(label))
            except:
                '''666'''
...
if __name__ == "__main__":
    ...
    parser.add_argument('--engine', nargs='+', type=str, default="/home/pip-2/RM/tensorrtx-yolov5-v5.0/yolov5/builds/best.engine", help='.engine path(s)')
    parser.add_argument('--save', type=int, default=0, help='save?')
    opt = parser.parse_args()
    PLUGIN_LIBRARY = "/home/pip-2/RM/tensorrtx-yolov5-v5.0/yolov5/builds/libmyplugins.so"
    ...
    try:
        ...
        while 1:
            ...
            if couunt_get_frame % 15 == 0 and couunt_get_frame > 1800 and pic_num <1000 :
            ...
            print(f"time is{end*1000-begin*1000}ms")
    except:
        'g'
